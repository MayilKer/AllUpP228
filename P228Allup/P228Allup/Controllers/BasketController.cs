    ...
    public class BasketController : Controller
    {
        ...
        public async Task<IActionResult> DeleteFromBasket(int? id)
        {
            ...
            if (!string.IsNullOrWhiteSpace(basket))
            {
                ...
                BasketVM basketPro = products.FirstOrDefault(p => p.Id == id);

                if (basketPro == null) return NotFound();

                products.Remove(basketPro);
            }
            else
            {
                BadRequest();
            ...
            foreach (BasketVM basketVM in products)
            {
                Product pro = await _context.Products.FirstOrDefaultAsync(p => p.Id == basketVM.Id && p.IsDeleted == false);

                basketVM.Title = pro.Title;
                basketVM.Image = pro.MainImage;
                basketVM.ExTax = pro.ExTax;
                basketVM.Price = pro.DiscountedPrice > 0 ? product.DiscountedPrice : product.Price;
            }
